...
const Events = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        const response = await axios.get('https://event-server-1-t82h.onrender.com/api/events', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currentEvent) {
        await axios.put(`https://event-server-1-t82h.onrender.com/api/events/${currentEvent._id}`, formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
      } else {
        await axios.post('https://event-server-1-t82h.onrender.com/api/events', formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
  ...
  const handleDelete = async (eventId) => {
    const token = localStorage.getItem('token');
    try {
      await axios.delete(`https://event-server-1-t82h.onrender.com/api/events/${eventId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
